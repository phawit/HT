...
var chart = new CanvasJS.Chart("chartContainer", {
	title:{
    text: "ข้อมูลอุณหภูมิ และความชื้นสัมพัทธ์",
    fontSize: 20
  }
});
chart.render();
...
function UpdateChart(){
  ...
  if (title_idx >= 0){
    chart = CreateChart(data_plot, chart, plot_last, title_idx);
    chart.render();
  }
}
...
function CreateChart(data_plot, chart, plot_last, title_idx) {
  // var date_plot = [];
  var humid_data = [];
  var temp_data = [];
  var hid_data = [];
  ...
  for (var i = data_plot.length-1; i >= 0 ; i--) {
    ...
    //if ((curr_date - new_date) < plot_last && (curr_date - new_date) >= 0){
    if ((curr_date - new_date) < plot_last){
        // alert(data_plot[i][0]+'xx'+new_date.toString()+'yy'+curr_date.toString()+'yy'+(curr_date - new_date).toString());
        tr = null;
...
ref.on('value',function (snapshot) {
  ...
  snapshot.forEach(function(childSnapshot) {       
    ...
    db_id.on('value',function (snapshot_id) {
      ...
      if ('DateTime' in data && 'Temperature' in data && 'Humid' in data && 'HID' in data && 'Flag' in data)
      {
